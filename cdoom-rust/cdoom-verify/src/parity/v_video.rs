//! Pixel-buffer / palette / patch parity for `v_video` + `v_diskicon` (CDOOM-47).

use std::ffi::c_void;
use std::sync::Mutex;

const SCREENWIDTH: usize = cdoom_core::v_video::SCREENWIDTH;
const SCREEN_PIXELS: usize = SCREENWIDTH * cdoom_core::v_video::SCREENHEIGHT;
...
#[test]
fn v_palette_gamma_helper_matches_c_palette_math() {
    let source_rgb = [0, 1, 2, 63, 127, 191, 252, 253, 255];
    let gamma_table = synthetic_gamma_table();
    let mut corrected = [0; 9];

    cdoom_core::cdoom_rust_v_gamma_correct_palette(
    ...
    let expected: Vec<u8> = source_rgb
        .iter()
        .map(|component| gamma_table[3 * 256 + usize::from(*component)] & !3)
        .collect();
    assert_eq!(corrected, expected.as_slice());
}
...
fn synthetic_gamma_table() -> Vec<u8> {
    let mut gamma = vec![0; 5 * 256];
    for level in 0..5 {
        for value in 0..256 {
            gamma[level * 256 + value] = (value as u8).wrapping_add((level * 17) as u8);
        }
    }
    gamma
}
