...
use crate::v_patch::{self, PatchPtr};
use std::ptr;
use std::sync::Mutex;
...
fn draw_patch_inner(x: i32, y: i32, patch: PatchPtr, flipped: bool, blend: PatchBlend) {
    let state = STATE.lock().expect("v_video state poisoned");
    let draw_x = x - v_patch::left_offset(patch);
    let draw_y = y - v_patch::top_offset(patch);
    let width = v_patch::width(patch);
    let height = v_patch::height(patch);
    ...
    for col in 0..width {
        ...
        let column_offset = v_patch::column_offset(patch, source_col);
